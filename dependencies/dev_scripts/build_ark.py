...
def build_patch_ark(xbox: bool):
    ...
    print("Building Dance Central 3 Deluxe patch arks...")
    ...
    if platform == "win32":
        build_location = "_build\\gen" if xbox else "_build\ps3\\USRDIR\gen"
    else:
        build_location = "_build/gen" if xbox else "_build/ps3/USRDIR/gen"
    ...
    # pull the latest changes from the Dance Central 3 Deluxe repo if necessary
    if not check_git_updated(repo_url="https://github.com/hmxmilohax/dance-central-3-deluxe", repo_root_path=root_dir):
        cmd_pull = "git pull https://github.com/hmxmilohax/dance-central-3-deluxe main".split()
    ...
    if not failed:
        print("Successfully built Dance Central 3 Deluxe ARK.")
        return True
    else:
